function time = query_groundtruth_ourspm_nhi(dict_words, inv_file, dict_structure, pparams, groundtruthDir, datasetDir)
...
%returns: after ranking, this function will run script to compute mAP from result directory
    ...
    for k = 1:length(q_files)
        ...
        script = [script_file, ...
            ...
            ' >result\paris6k\result-30i-5top-ourspmlv1-weight-nhi-23-05\', image_name(6:end), '_result.txt']; %q_files(k).name(1:end-10)
        system(script);
        if verbose==1
            pause;
        end
