function time =  query_groundtruth_SPM_myinv(dict_words, inv_file, dict_structure, pyramid_all, pyramid_params, groundtruthDir, datasetDir)
...
%returns: after ranking, this function will run script to compute mAP from result directory
    ...
    for k = 1:length(q_files)
        ...
        tic;
        indexes = inv_search_c(inv_file, q_words{1});
        
        %ranked_ids = indexes{1};
        ids = ranking_spm(pyramid_all, q_ids, q_values, indexes);
        time(k) = toc;
        ...
        script = [script_file, ...
            ...
            ' >result\oxford5k\result-30i-5top-spmlv2-nhi-24.5\', image_name(1:end), '_result.txt']; %q_files(k).name(1:end-10)
        system(script);
        if verbose==1
            pause;
        end
