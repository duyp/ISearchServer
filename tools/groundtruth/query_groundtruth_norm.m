function time = query_groundtruth_norm(dict_words, inv_file, dict_structure, groundtruthDir, datasetDir)
...
%returns: after ranking, this function will run script to compute mAP from result directory
    ...
    %for k = 1:length(q_files)
        ...
        tic;
        [ids, ~] = inv_search_norm_c(inv_file, q_words{1});
        time(k) = toc;
        ...
        script = [script_file, ...
            ...
            ' >result\oxford5k\result-30i-5top-nhi-22.5\', image_name(6:end), '_result.txt']; %q_files(k).name(1:end-10)
        system(script);
        if verbose==1
            pause;
        end
