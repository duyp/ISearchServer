...
#define COMPUTE_L1(d, data1, data2)                     \
  ...
  if (sparse)                                                               \
    ...
    {                                                             \
      d += (Tret)fabs((float)data1.getPointVal(col1,row) - data2.getPointVal(col2,row));   \
    }                                                                           \
  else      \
  {\
    pair<T*, uint> point1 = data1.getPoint(col1), point2=data2.getPoint(col2);\
    for (row=0; row<rows1; ++row)                         \
      d += (Tret)fabs((float)(point1.first[row]) - (point2.first[row]));   \
  }
...
#define COMPUTE_KL(d, data1, data2)                           \
  ...
  {                                                             \
    ...
    q2 = t2/t1;                                                 \
    /*compute*/                                                 \
    d += (t1 * log((double)q1) / log(2.0)) + (t2 * log((double)q2) / log(2.0));     \
  }                                                             \
  d /= 2;
