...
//    mexErrMsgTxt("Unknown distance function.");
  ...
  //allocate output
//           mxDuplicateArray(dataIn);  //(mxArray*)dataIn; //
...
#define __CASE(CLASS)                                                   \
  {                                                                     \
    ...
    /*output*/                                                          \
    normOut = mxCreateNumericMatrix(1, data.size(), mxSINGLE_CLASS, mxREAL); \
    Vector<float> ret;                                                  \
    fillMatrix(ret, normOut);                                           \
    /*normalize*/                                                       \
    norm(data, metric, ret);                                            \
  }
