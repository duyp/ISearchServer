function executeQueryThreadNormal(id, dict, inv, dict_structure, datasetDir, datasetFiles)
    service = javaObjectEDT('com.uit.instancesearch.services.ISService');
    while 1
        %try
            %tic;
        ...
        tic;
        try
            result = clientQuery(queryContent,dict,inv,dict_structure,datasetDir,datasetFiles,50,15);
            if isempty(result{1})
                result{1} = 'NULL';
            end
        catch
